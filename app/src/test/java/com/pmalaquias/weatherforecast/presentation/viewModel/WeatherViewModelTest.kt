...
class WeatherViewModelTest {
    ...
    fun `init - GIVEN repository success - THEN uiState is updated with data`() = runTest {
        ...
        val expectedWeatherData = WeatherData(expectedLocation, current = expectedCurrentWeather)
        ...
        coEvery { weatherRepository.getCurrentWeatherData(any()) } returns expectedWeatherData
        coEvery {
            weatherRepository.getForecastData(
                any(),
                any()
            )
        } returns expectedForecastData
    ...
    fun `init - GIVEN repository failure - THEN uiState is updated with error`() = runTest {
        ...
        coEvery { weatherRepository.getCurrentWeatherData(any()) } returns null
        coEvery { weatherRepository.getForecastData(any(), any()) } returns null
